...
const GroupLayer = (props: IProps) => {
  const { editorStore, pageTreeStore } = useStore();
  const { forceUpdateAttr } = editorStore;
  const { item = {}, keyWord } = props;
  const { onContextMenu, selectedKeys, changeInstance, dragEnd, dragStart, keyIndex: key, dragCount } = props;
  // 组件是否上锁
  ...
  useEffect(() => {
    if (isInvisible !== item.comInvisible) {
      ...
      // console.log('useEffect setIsInvisible', item.comInvisible);
    }
  ...
  return (
    <>
      ...
      >
        ...
        <div className='bottom-part-line' />
      </div>
      {/* 组是否展开 */}
      {isOpened &&
...
export default observer(GroupLayer);

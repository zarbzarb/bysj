...
const UpdateData = ({ comp, parentIdx, idx }) => {
  ...
  if (item?.actionSettings?.compKey) {
    ...
    if (v) {
      if (v.classType === 'com' && v.instance.config._source !== 'json') {
        item.actionSettings.compKey = '';
      } else if (v.classType === 'antd') {
        if (v.type === 'LayerLegend' && v.props.dataSourceSet.category !== 'json') {
          item.actionSettings.compKey = '';
        }
        if (v.type === 'Calendar') {
          if (v.dataset.isVariable) {
            item.actionSettings.compKey = '';
          }
        } else if (v?.dataset?.category !== 'json' && v.type !== 'LayerLegend') {
          item.actionSettings.compKey = '';
        }
      }
    } else if (v && v.classType === 'customComp' && v?.dataset?.category !== 'json') {
      item.actionSettings.compKey = '';
    } else if (pageInfoMap[item.actionSettings.appPageId] && !v) {
      item.actionSettings.compKey = '';
    }
  }
  ...
  const showEditorParams = () => {
    ...
    if (v.classType === 'com') {
      ...
      const { _seriesType = 2, _api, _dataMap, dynamic } = v.instance?.config;

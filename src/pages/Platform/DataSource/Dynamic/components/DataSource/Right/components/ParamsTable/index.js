...
import { Row, Col, Table, Popover, Input, Tooltip, Checkbox } from 'antd';
...
function ParamsTable(props) {
  const { onChange, value, isHeader = false } = props;
  ...
  const onChangeValue = (v, i) => {
    ...
    //   draft[i].example = v;
    // });
    const newV = value.map((item) => ({ ...item }));
    newV[i].status = v !== newV[i].defaultValue;
    newV[i].example = v;
    onChange(newV);
  };
  const onChangeHeaderValue = (v, i) => {
    // const newV = produce(value, (draft) => {
    //   draft[i].value = v;
    // });
    const newV = value.map((item) => ({ ...item }));
      ...
      render: (text, record, index) => {
        return (
          <EditInput
            ...
            verify={false}
            onChange={(v) => onChangeHeaderValue(v, index)}
          />
        );
      },
    },
  ...
  const columns = [
...

    {
      ...
      align: 'center',
      render: (text, record, index) => {
        return <Checkbox key={record.key || record.id} checked={text} disabled />;
      },
    },
  ...
  return (
    <div>
      ...
      <Table
        columns={isHeader ? headerColumns : columns}
        dataSource={value}
        pagination={false}
        bordered
        rowKey='id'

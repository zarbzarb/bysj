...
// 不定长度的组件，根据传入值值来判断循环谁
const infiinteLengthComps = ['Table'];
...
export const replaceKeyDataDispose = (paramType, val, dataset, triggerComp) => {
  console.log(paramType, val, dataset, triggerComp);

  // 触发对象的映射字段
  const triggerKeys = dataset._api.map((item) => item.mapField);
  ...
  console.log(result, 'result', triggerKeys);

  const res = result.filter((item) => {
    let isBool = false;
    for (const i in item) {
      if (triggerKeys.includes(i) && (item[i] || item[i] === 0)) {
        isBool = true;
        break;
      }
    }
...
export const antdDataDispose = (paramType, val, dataset, triggerComp) => {
  ...
  console.log(dataset, 'dataset', triggerComp);
...
/**
 *
 * @param {*} paramType 触发对象的key
 * @param {*} val 要赋值的数组
 * @param {*} dataset 触发对象的dataset
 * @param {*} triggerComp 触发对象的组件实例
 */
export const setDataset = (paramType, val, dataset, triggerComp) => {
  const _val = typeof val === 'object' ? val : [val];

  if (triggerComp.classType === 'com') {
    if (CompatibleTool.isXYSChart(dataset)) {
      const { _seriesType = 2 } = triggerComp.instance?.config;
      if (_seriesType === 2) {
        replaceKeyDataDispose(paramType, _val, dataset, triggerComp);
      } else {
        xysDataDispose1(paramType, _val, dataset, triggerComp);
      }
    } else if (triggerComp.englishName === 'ChartNestRing') {
      chartNestDataDispose(paramType, _val, dataset, triggerComp);
    } else {
      replaceKeyDataDispose(paramType, _val, dataset, triggerComp);
    }
  ...
  if (triggerComp.classType === 'antd') {
    antdDataDispose(paramType, _val, dataset, triggerComp);
    return;
  }

  if (triggerComp.classType === 'customComp') {
    antdDataDispose(paramType, _val, dataset, triggerComp);
    return;
  }
};
...
export const render = (triggerComp, dataset) => {
  if (['antd', 'customComp'].includes(triggerComp.classType)) {
    // 为了useComponentDataSource里的 useEffect 能触发, 需要重新赋值一下！
    dataset.dynamic = { ...dataset.dynamic };
    if (typeof triggerComp.refresh === 'function') {
      triggerComp.refresh();
    }
